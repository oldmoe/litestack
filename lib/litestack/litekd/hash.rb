module Litekd
  class Hash < Composite
    ...
    def push(hkey, value) = conn.write_composite_member(@key, hkey.to_s, dump(value))
    ...
    def remove_key(hkey) = conn.remove_composite_member_by_position(@key, hkey)
    def remove(hkey = nil) = hkey ? remove_key(hkey) : conn.delete_composite_structure(@key)    
